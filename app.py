...
def update_entry(entry_id: int, entry: dict):
    ...
    sql = """
        UPDATE entries SET
            date=?, time_slot=?, account=?, campaign=?, vertical=?, pml_code=?,
            platform=?, brands=?,
...
.acct-panel-head {{
    background: linear-gradient(90deg, #c5163f 0%, #6b0a1f 100%);
    padding: 12px 18px;
    color: #ffffff;
    font-weight: 700;
    font-size: 0.85rem;
...
.acct-panel-head.pnl {{
    background: linear-gradient(90deg, #2a0d18 0%, #1a0a12 100%);
    color: #ff3b6b;
    border-bottom: 1px solid #3a1525;
}}
...
.acct-table tr.total-row td {{
    ...
    border-top: 2px solid #ff3b6b;
    color: #ff3b6b;
    font-weight: 700;
}}
.acct-table tr.total-row td.label {{ font-weight: 800; letter-spacing: 0.06em; }}
.acct-table .num-pos {{ color: #34d399; font-weight: 600; }}
.acct-table .num-neg {{ color: #f87171; font-weight: 600; }}
.acct-table .num-rev {{ color: #fbbf24; font-weight: 600; }}
.acct-table .muted {{ color: {t['text_dim']}; }}
.acct-table .row-idx {{ color: #ff3b6b; font-weight: 600; }}
...
def render_member_account_view(df_all: pd.DataFrame, users: dict):
    """Picture-style layout: account pills → per-account entries panel + PNL panel."""
    ...
    for idx, (tab, acct) in enumerate(zip(tabs, accounts), start=1):
        with tab:
            ...
            for i, (_, r) in enumerate(sub.iterrows(), start=1):
                ...
                tag = r.get("PML Code") or r.get("Vertical") or "—"
                spend = float(r.get("Spend", 0) or 0)
                revenue = float(r.get("Revenue", 0) or 0)
                fb_clicks = int(r.get("Clicks", 0) or 0)
                lp_clicks = int(r.get("LP Clicks", 0) or 0)
                conv = int(r.get("Conversions", 0) or 0)
                updated = _fmt_dt(r.get("Timestamp"))
                rows_html.append(
                    f"<tr>"
                    f"<td class='row-idx'>{idx}.{i}</td>"
                    f"<td>{person}</td>"
                    f"<td>{campaign}</td>"
                    f"<td class='muted'>{tag}</td>"
                    f"<td>{_fmt_money(spend)}</td>"
                    f"<td class='num-rev'>{_fmt_money(revenue)}</td>"
                    f"<td>{_fmt_int(fb_clicks)}</td>"
                    f"<td>{_fmt_int(lp_clicks)}</td>"
                    f"<td>{_fmt_int(conv)}</td>"
            ...
            # ── Bottom panel: PNL summary ──────────────────────────────────
            pnl_head = (f"<div class='acct-panel'>"
                        f"<div class='acct-panel-head pnl'>📊 ACCOUNT {idx} PNL</div>")

            pnl_html = ["<table class='acct-table'>",
                        "<thead><tr>"
                        "<th>#</th><th>Person</th><th>Campaign</th><th>Tag</th>"
                        "<th>Spend</th><th>Revenue</th><th>PNL</th><th>ROI</th>"
                        "<th>FB Clicks</th><th>LP Clicks</th><th>Conv.</th>"
                        "<th>CPC</th><th>LP CTR</th><th>Offer CR</th>"
                        "<th>Updated</th><th>Status</th>"
                        "</tr></thead><tbody>"]

            tot_spend = tot_rev = tot_profit = 0.0
            tot_fb = tot_lp = tot_conv = 0
            ...
            for i, (_, r) in enumerate(sub.iterrows(), start=1):
                ...
                lp_ctr = float(r.get("LP CTR", 0) or 0)
                offer_cr = float(r.get("Offer CR", 0) or 0)
                updated = _fmt_dt(r.get("Timestamp"))

                tot_spend += spend; tot_rev += revenue; tot_profit += profit
                tot_fb += fb_clicks; tot_lp += lp_clicks; tot_conv += conv

                profit_cls = "num-pos" if profit >= 0 else "num-neg"
                status = ("🟢 Profit" if profit > 0
                          else "🔴 Loss" if profit < 0 else "⚪ Even")
                ...
                pnl_html.append(
                    ...
                    f"<td>{_fmt_money(spend)}</td>"
                    f"<td class='num-rev'>{_fmt_money(revenue)}</td>"
                    f"<td class='{profit_cls}'>{_fmt_money(profit)}</td>"
                    f"<td class='{profit_cls}'>{_fmt_pct(roi)}</td>"
                    f"<td>{_fmt_int(fb_clicks)}</td>"
                    f"<td>{_fmt_int(lp_clicks)}</td>"
                    f"<td>{_fmt_int(conv)}</td>"
                    f"<td>{_fmt_money(cpc)}</td>"
                    f"<td>{_fmt_pct(lp_ctr)}</td>"
                    f"<td>{_fmt_pct(offer_cr)}</td>"
                    f"<td class='muted'>{updated}</td>"
                    f"<td>{status}</td>"
                    f"</tr>"
                )

            tot_roi = (tot_profit / tot_spend * 100) if tot_spend > 0 else 0
            tot_cpc = (tot_spend / tot_fb) if tot_fb > 0 else 0
            tot_profit_cls = "num-pos" if tot_profit >= 0 else "num-neg"
            pnl_html.append(
                f"<tr class='total-row'>"
                f"<td class='label'>{idx}</td>"
                f"<td class='label'>TOTAL</td>"
                f"<td></td><td></td>"
                f"<td>{_fmt_money(tot_spend)}</td>"
                f"<td>{_fmt_money(tot_rev)}</td>"
                f"<td class='{tot_profit_cls}'>{_fmt_money(tot_profit)}</td>"
                f"<td class='{tot_profit_cls}'>{_fmt_pct(tot_roi)}</td>"
                f"<td>{_fmt_int(tot_fb)}</td>"
                f"<td>{_fmt_int(tot_lp)}</td>"
                f"<td>{_fmt_int(tot_conv)}</td>"
                f"<td>{_fmt_money(tot_cpc)}</td>"
    ...
    with st.expander("➕ Add Multiple Entries (Bulk)", expanded=False):
        ...
        if save_clicked:
            ...
            for _, r in edited.iterrows():
                ...
                spend = float(r.get("Spend", 0) or 0)
                revenue = float(r.get("Revenue", 0) or 0)
                if not acct or (spend == 0 and revenue == 0):
                    skipped += 1
                    continue
            ...
            # Reset the editor to a fresh blank scaffold
            st.session_state["bulk_rows_df"] = pd.DataFrame([
                {"Account": "", "Campaign": "", "Tag": "",
                 "Spend": 0.0, "Revenue": 0.0,
                 "FB Clicks": 0, "LP Clicks": 0,
            ...
            # Clear the editor's own state so it picks up the new scaffold
            ...
            if skipped: msg_parts.append(f"⏭️ Skipped {skipped} empty row{'s' if skipped != 1 else ''}")
            if new_accts: msg_parts.append(f"🆕 Auto-added accounts: {', '.join(new_accts)}")
            if msg_parts:
                st.success("  ·  ".join(msg_parts))
                st.rerun()
            else:
                st.warning("Nothing to save — fill in at least one row with an Account and Spend or Revenue > 0.")
...
def my_dashboard_page():
    ...
    if role == "member":
        ...
        render_member_account_view(df_all, users)
