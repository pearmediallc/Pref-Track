...
def sidebar():
    with st.sidebar:
        ...
        <div style='margin-bottom:20px;'>
            ...
            <span style='color:#6b7280; font-size:0.8rem;'>{sub}</span>
        </div>
        ...
        if role == "admin":
            ...
            pages = ["📝 Add Metrics", "📈 My Dashboard"]
...
def add_data_page():
    ...
    with st.form("add_data_form", clear_on_submit=True):
        ...
        with c2:
            account = st.text_input("Account Name *", placeholder="e.g. Plum-0902226-001")
            campaign = st.text_input("Campaign", placeholder="e.g. Spring-Sale-CA")
        with c3:
            vertical = st.text_input("Vertical", placeholder="e.g. Finance")
            pml_code = st.text_input("PML Code", placeholder="e.g. PML-0912")

        c4, c5 = st.columns(2)
        with c4:
            platform = st.text_input("Platform", placeholder="e.g. Meta")
        with c5:
            brands = st.text_input("Advertiser", placeholder="e.g. Acme Corp")
        ...
        if submitted:
            if not account.strip():
                ...
                insert_entry(entry)
                st.success(f"✅ Entry saved — {account} · {time_slot} · {entry_date}")

    # Recent entries for this user
    df = fetch_entries(media_buyer=st.session_state.username)
    if not df.empty:
...
def master_dashboard_page():
    ...
    # Filters: team + account
    fc1, fc2 = st.columns(2)
    with fc1:
        leads_in_data = sorted(df_all["team_lead"].dropna().unique().tolist())
        team_options = ["(All teams)"] + leads_in_data
        picked_team = st.selectbox(
            "🏷️ Filter by Team Lead", team_options,
            format_func=lambda u: u if u == "(All teams)"
                                    else users.get(u, {}).get("display_name", u or "—"))
    with fc2:
        scope = None if picked_team == "(All teams)" else picked_team
        picked_account = account_filter(scope_lead=scope, key="master_acct_filter")

    df = df_all
    if picked_team != "(All teams)":
        df = df[df["team_lead"] == picked_team]
    if picked_account:
        df = df[df["Account"] == picked_account]
    ...
    # Team comparison (use df_all so the comparison shows all teams regardless of team filter)
    base = df_all if not picked_account else df_all[df_all["Account"] == picked_account]
    ...
    fig3.update_layout(paper_bgcolor="#0d0f14", plot_bgcolor="#0d0f14",
                       font=dict(color="#e8eaf0"), height=360,
                       xaxis=dict(gridcolor="#1e2235"), yaxis=dict(gridcolor="#1e2235"),
                       legend=dict(bgcolor="#141824", bordercolor="#252a3d"))
    st.plotly_chart(fig3, use_container_width=True)
